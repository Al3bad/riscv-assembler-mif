...
class AssemblyConverter:
	...
	def __init__(self, output_type='-', nibble = False, filename = "", hexMode = False):	
		self.code = []
		self.instructions = []
		self.hexMode = hexMode

		if "b" not in output_type and "t" not in output_type and "p" not in output_type and "-" not in output_type:
	...
	def __pre(self):
		...
		rmap_path = self.resource_path(os.path.join("data", "reg_map.dat"))	
		r_p = {}
		...
		i_data = {}
		instr_path = self.resource_path(os.path.join("data", "instr_data.dat"))
		f = open(instr_path,"r")
